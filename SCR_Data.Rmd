...
*NOTE: The z-standardization of the SCR (variable iscr_gng_resp_sqrt_z_score) is based on only trials that also enter the analyses, not on all trials. This means, that e.g. word and gng outliers, trials with miss or wrong key in word/gng response, incorrect word responses, and trials followed_or_preceded_by_FA_or_wrong_key were not included when scaling the SCR. I once tried basing it on all trials. The only things that change are the values of the SCR ANOVAs and Raw-Barplots based on this variable. The results of the LMM on the Relation Priming - SCR remain the same, only some decimals are slightly different. I thus decided to stick with the scaling based on only the trials that enter the analysis, because distribution of the variable is then a it nicer (when basing it on all trials, I again get a peak at the value of 0) and basing the scaling on all trials does not reflect the true value better. For the manuscript, I calculate the variable iscr_gng_resp_sqrt_z_score first when calculating the LMM Priming - SCR, to avoid having the variable containing many NAs in the Data I publish.*
...
#### Create variable with color of fixation cross (needed for control analysis) -> has to be done before cleaning data!!!
data4mixedmodels$color_cross <- NA

# Which rows satisfy the criteria? which rows are fast hits?
indices_FH    <- which(data4mixedmodels$gng_resp == 41)
indices_SH_FA <- which(data4mixedmodels$gng_resp > 41)  # specify second condition

# Assign 0 to each row following FH except very last row
data4mixedmodels[1:(nrow(data4mixedmodels)+1),]$color_cross[indices_FH+1] <- 0     # white cross in FH trials (CI not included here)
data4mixedmodels[1:(nrow(data4mixedmodels)+1),]$color_cross[indices_SH_FA+1] <- 1  # red cross in FA / SH trials

# Delete empty rows that were assigned to df now
data4mixedmodels <- data4mixedmodels[!is.na(data4mixedmodels$subjectID),]

# First trial of each block starts with  white fixcross
data4mixedmodels[data4mixedmodels$trial == 1 | data4mixedmodels$trial == 29 | data4mixedmodels$trial == 101 | 
                   data4mixedmodels$trial == 173 | data4mixedmodels$trial == 201 | data4mixedmodels$trial == 273 | 
                   data4mixedmodels$trial == 345 | data4mixedmodels$trial == 373 | data4mixedmodels$trial == 445,]$color_cross <- 0
...
plot_est_means_scr <- ggplot(Estimated_Means_SCR,aes (x = response_type,y = response)) +
 ...
 # scale_fill_grey(start = 0.40, end = 0.6) + # greyscales 
...
plot_est_means_scr
ggsave("figure_scr.tiff",width = 10, height = 7, units = "cm", dpi=600)
...
As bivariate normality is almost given (see last line), a Pearson correlation is calculated. 
...
tab_model(LMM_scr_final,order.terms = c(1,2,6,5,4,3), dv.labels = "SCR [square root transformed (µS)]", pred.labels = pl,wrap.labels = 60, show.stat = TRUE, show.se = TRUE, string.se = "SE", show.loglik = TRUE, show.dev = TRUE, show.re.var = FALSE, show.ngroups = FALSE, show.icc = FALSE, show.r2 = TRUE)
...
LMM_scr_cross_clean <- lmer(iscr_gng_resp_sqrt ~ FH_minus_SH + FA_minus_FH + red_minus_white + FH_minus_SH:red_minus_white + FA_minus_FH:red_minus_white + (1 + FA_minus_FH + red_minus_white || subjectID), data=data4mixedmodels_words_scr[data4mixedmodels_words_scr$word_accuracy==1 & data4mixedmodels_words_scr$followed_or_preceded_by_red_cross == FALSE,], REML = TRUE, control=lmerControl(optimizer="bobyqa"))

tab_model(LMM_scr_cross_clean,dv.labels = "SCR [square root z transformed (µS)]", pred.labels = pl, wrap.labels = 60, show.stat = TRUE, show.se = TRUE, string.se = "SE", show.loglik = TRUE, show.dev = TRUE, show.re.var = FALSE, show.ngroups = FALSE, show.icc = FALSE, show.r2 = TRUE)


# LMM use all trials
LMM_scr_cross <- lmer(iscr_gng_resp_sqrt ~ FH_minus_SH + FA_minus_FH + red_minus_white + FH_minus_SH:red_minus_white + FA_minus_FH:red_minus_white + (1 + FH_minus_SH + FA_minus_FH + red_minus_white + FH_minus_SH:red_minus_white || subjectID), data=data4mixedmodels_words_scr[data4mixedmodels_words_scr$word_accuracy==1,], REML = TRUE, control=lmerControl(optimizer="bobyqa"))

tab_model(LMM_scr_cross, dv.labels = "SCR [square root transformed (µS)]", pred.labels = pl, wrap.labels = 60, show.stat = TRUE, show.se = TRUE, string.se = "SE", show.loglik = TRUE, show.dev = TRUE, show.re.var = FALSE, show.ngroups = FALSE, show.icc = FALSE, show.r2 = TRUE)
```
