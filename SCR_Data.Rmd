...
df_aggregated_scr_condition_over_subjects <- df_aggregated_scr_condition %>%
  group_by(condition) %>%
    summarise_at(.vars = "iscr_gng_resp_sqrt_z_score", funs(mean,sd,se=sd(.)/sqrt(n())))

df_aggregated_scr_condition_over_subjects$response_type <- factor(substr(df_aggregated_scr_condition_over_subjects$condition, 11, 12), levels=c("FA","FH","SH")) 
df_aggregated_scr_condition_over_subjects$word_valence  <- factor(substr(df_aggregated_scr_condition_over_subjects$condition, 1, 3))

# aggregating over subjects per response type (needed for barplots)
df_aggregated_scr_resp_type_over_subjects <- df_aggregated_scr_response_type %>%
  group_by(response_type) %>%
    summarise_at(.vars = "iscr_gng_resp_sqrt_z_score", funs(mean,sd,se=sd(.)/sqrt(n())))

df_aggregated_scr_resp_type_over_subjects$response_type <- factor(df_aggregated_scr_resp_type_over_subjects$response_type, levels=c("FA","FH","SH")) 
...
normality <- do.call(rbind, lapply(df_wide_scr[,c("iscr_gng_resp_sqrt_z_score.FA","iscr_gng_resp_sqrt_z_score.FH","iscr_gng_resp_sqrt_z_score.SH","iscr_gng_resp_sqrt_z_score.neg_after_FA","iscr_gng_resp_sqrt_z_score.pos_after_FA","iscr_gng_resp_sqrt_z_score.neg_after_FH","iscr_gng_resp_sqrt_z_score.pos_after_FH","iscr_gng_resp_sqrt_z_score.neg_after_SH","iscr_gng_resp_sqrt_z_score.pos_after_SH")], function(x) shapiro.test(x)["p.value"]))
...
Non-responder describes an individual who fails to show any or a certain (arbitrary) percentage of non-zero CRs (Lonsdorf et al., 2017). Here, participants lacking measurable SCR on >75% of all trials (or trials of one response condition) were classified as non-responders.All participants' SCRs exceeded this threshold so no participants were excluded.

```{r non-responders SCR}

df_aggregated_scr_response_type_excl_zero_resp <- ddply(data4mixedmodels_words_scr[data4mixedmodels_words_scr$word_accuracy==1 & data4mixedmodels_words_scr$iscr_gng_resp > 0,], 
                                                        .(subjectID, response_type), 
                                                        summarise,
                                                        event_counts_excl_zero_scr = length(subjectID))

df_non_responder_per_response_type <- left_join(df_aggregated_scr_response_type,df_aggregated_scr_response_type_excl_zero_resp, by = c("subjectID","response_type"))

df_non_responder_per_response_type$proportion_zero_responses <- round(100 - (df_non_responder_per_response_type$event_counts_excl_zero_scr / df_non_responder_per_response_type$event_counts_incl_zero_scr * 100),digits = 1)
...
df_non_responder_overall$proportion_zero_responses_overall    <- round(100 - (df_non_responder_overall$number_all_events_excl_zero_scr / df_non_responder_overall$number_all_events_incl_zero_scr * 100),digits = 1)
...
df_non_responder_per_response_type <- df_non_responder_per_response_type["proportion_zero_responses"]
df_non_responder_per_response_type <- cbind(df_non_responder_per_response_type[1:30,],df_non_responder_per_response_type[31:60,],df_non_responder_per_response_type[61:90,])
colnames(df_non_responder_per_response_type) <- c("proportion_zero_responses_FA","proportion_zero_responses_FH","proportion_zero_responses_SH")
...
I chose sliding difference (aka "repeated") contrasts, which successively test neighboring factor levels against each other. See more information on that topic in the section "Word Categorization Behavioral Data".
...
For the SCR, the final model that converged and had the best fit included a fully specified by-subject random structure (random intercepts and random slopes for all fixed effects, including correlation parameters). This is also the final model I get when I initially include (1 + FH_minus_SH + FA_minus_FH  | word) as random effect in the maximal model.


```{r build LMM SCR, echo = 2}

LMM_scr_no_val_final <- lmer(iscr_gng_resp_sqrt ~ FH_minus_SH + FA_minus_FH + (1 + FH_minus_SH + FA_minus_FH  | subjectID), data=data4mixedmodels_words_scr[data4mixedmodels_words_scr$word_accuracy==1,], REML = TRUE, control=lmerControl(optimizer="bobyqa"))
summary(LMM_scr_no_val_final)                         # model does converge, no singular fit -> model is identified

LMM_scr_no_val_final_summary <- summary(LMM_scr_no_val_final)

kable(LMM_scr_no_val_final_summary$coefficients, caption = "Overview Fixed Effects LMM SCR") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "left")
```
<br>

Akin to the ANOVA omnibus test, it can be tested whether the main effect of response type improves the model fit significantly. It does. Thus, there is an omnibus main effect of response type. 

<br>

#### Summary LMM

The SCR differs significantly between FAs and FHs but not between FHs and SHs (but there is a trend). These results are  consistent with the ANOVA post-hoc tests. Important note: the trend was not present (neither for LMM not for ANOVA) when I did not exclude the word outliers for these analyses. In the GLMM, which shows mostly consistent results, FH vs SH is not significant or a trend even after exclusion of trials with word outlier.
...
anova_accuracy_posthoc_resp   <- emmeans(anova_SCR, ~ response_type)
anova_accuracy_posthoc_resp   <- pairs(anova_accuracy_posthoc_resp)
...
Residuals of the square root transformed SCR are normally distributed in each condition, exept for FA (pos). But even in this condition, the deviation is not severe.
...
To check whether the SCR is affected by the word following the GNG response, I also conducted a LMM and an ANOVA including the factors response type and valence of the following word (levels neg, pos). The results can be reported as a footnote: There is still a main effect of Response Type. There is no main effect of valence and in interaction of valence and response type. Hence, the SCR is not influenced by valence of following word. The results of the LMM and the ANOVA including response type and valence are mostly consistent (as before FH vs SH is a trend in the LMM, but in the ANOVA it gets significant here).

```{r test effect of valence on SCR}

# LMM
LMM_scr_final <- lmer(iscr_gng_resp_sqrt ~ response_type*valence + (1 + FH_minus_SH + FA_minus_FH | subjectID), data=data4mixedmodels_words_scr[data4mixedmodels_words_scr$word_accuracy==1,], REML = TRUE, control=lmerControl(optimizer="bobyqa"))
#summary(LMM_scr_final)            # model does converge, no singular fit -> model is identified

LMM_scr_final_summary <- summary(LMM_scr_final)

kable(LMM_scr_final_summary$coefficients, caption = "Overview Fixed Effects LMM SCR Including Valence") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "left")
...
#### Influence of Color Fixation Cross on SCR

I cannot evaluate effect of color of fixation cross in a really clean way. For doing this, I would need to include trials with write and red crosses that were only preceded and followed by white crosses. If I do this, only 1205 trials remain for the analysis. There, there indeed is a trend for the effect of color of fixation cross on SCR. The effect actually is significant if I remove the random slope (which according to anova function does not improve the fit). But this analysis is based on a tiny subset of the original data and not representative.

Thus, I also checked whether the color of fixation cross affects the SCR in current trial (independent of color of fix cross in previous or following trial; note that the time window of SCR is much longer than current trial, leading to overlapping effects of subsequent fixation crosses). At least in this analysis, which allowed overlapping effects of fixation crosses of different color, the color of the fixation cross does not affect the SCR in the current trial.
...
# attach to dataframe 
data4mixedmodels_words_scr[,(ncol(data4mixedmodels_words_scr)+1):(ncol(data4mixedmodels_words_scr)+6)] <- mm_c2
names(data4mixedmodels_words_scr)[(ncol(data4mixedmodels_words_scr)-5):ncol(data4mixedmodels_words_scr)] <- c("Grand Mean", "FH_minus_SH","FA_minus_FH","red_minus_white", "FH_minus_SH:red_minus_white", "FA_minus_FH:red_minus_white")
...
LMM_scr_cross_clean <- lmer(iscr_gng_resp_sqrt ~ FH_minus_SH + FA_minus_FH + red_minus_white + FH_minus_SH:red_minus_white + FA_minus_FH:red_minus_white + (1 + FH_minus_SH:red_minus_white  || subjectID), data=data4mixedmodels_words_scr[data4mixedmodels_words_scr$word_accuracy==1 & data4mixedmodels_words_scr$followed_or_preceded_by_red_cross == FALSE,], REML = FALSE, control=lmerControl(optimizer="bobyqa"))

LMM_scr_cross_clean_summary <- summary(LMM_scr_cross_clean)

kable(LMM_scr_cross_clean_summary$coefficients, caption = "Overview Fixed Effects LMM SCR - Color Fix Cross") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "left")
...
LMM_scr_cross_summary <- summary(LMM_scr_cross)

kable(LMM_scr_cross_summary$coefficients, caption = "Overview Fixed Effects LMM SCR - Color Fix Cross") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "left")
```
...
ggplot(df_aggregated_scr_condition_over_subjects,aes (x = response_type,y = mean, fill = word_valence)) +
  geom_bar(stat="identity", position=position_dodge()) +                                                       
  geom_errorbar(aes(ymax = mean + se, ymin = mean - se), position = position_dodge(width=0.95), width=0.1) +    
  ggtitle("SCR Depending on Response Type and Following Word Valence") +                                                                            
  xlab("Previous Response") + ylab("SCR (square root and z transformed)") +                                                                  
  ...
  geom_signif(y_position=c(0.6,0.6), xmin=c(1.02, 2.02), xmax=c(1.98, 2.98),
              annotation=c("***","NS"), tip_length=0.02) 

# plot SCR per subject
ggplot(data=df_aggregated_scr_response_type, aes(x=response_type, y = iscr_gng_resp_sqrt_z_score))+geom_point()+facet_wrap(~subjectID)+theme_bw()+ ggtitle("Skin Conductance Response per Subject") + theme(plot.title = element_text(hjust = 0.5)) + ylab("Square Root-Transformed Z-Standardized SCR (?S)")



plot_scr <- ggplot(df_aggregated_scr_resp_type_over_subjects,aes (x = response_type,y = mean)) +
  geom_bar(stat="identity", position=position_dodge(), fill = "steelblue4") +                                                       
  geom_errorbar(aes(ymax = mean + se, ymin = mean - se), position = position_dodge(width=0.95), width=0.1) +    
  ggtitle("Skin Conductance Response") +                                                                            
  xlab("Response Type") + ylab("Sqrt-Transformed Z-Standardized SCR (?S)") +                                                                  
  guides(fill=guide_legend(title="Word Valence")) +                                                                         
  theme(plot.title = element_text(hjust = 0.5)) +
...
* Fixed effects: Response type (FA, FA, SH) and Word Valence (pos, neg)
    
* Random effects: Subjects and Words

* Continuous predictor: Square root transformed and z-standardized SCR (response-locked to GNG-response) (it is recommended that contonuous predictor shall be scaled)


As in ll other analyses on SCR data, I only included FA, FH, SH trials that had no invalid gng rt or word rt (these trials were consistently excluded for all analyses), were followed by correct word classification and were not preceded or followed by an incorrect response or wrong key in gng or word classification.
...
For the Priming - SCR LMM, the final model that converged and had the best fit included for the random effect of subject: random intercept and slope of response type, valence, response type*valence, and SCR (excluding correlation parameters). For the random effect of word it included the random intercept and slope of response type (excluding correlation parameters).

```{r build LMM Priming SCR, echo = 2}

LMM_rt_scr_final <- lmer(word_rt_inverse ~ FH_minus_SH + FA_minus_FH + pos_minus_neg + FH_minus_SH:pos_minus_neg + FA_minus_FH:pos_minus_neg + iscr_gng_resp_sqrt_z_score + FH_minus_SH:iscr_gng_resp_sqrt_z_score + FA_minus_FH:iscr_gng_resp_sqrt_z_score + pos_minus_neg:iscr_gng_resp_sqrt_z_score + FH_minus_SH:pos_minus_neg:iscr_gng_resp_sqrt_z_score + FA_minus_FH:pos_minus_neg:iscr_gng_resp_sqrt_z_score + (1 + FH_minus_SH + FA_minus_FH + pos_minus_neg + FH_minus_SH:pos_minus_neg + FA_minus_FH:pos_minus_neg + iscr_gng_resp_sqrt_z_score  || subjectID) + (1 + FH_minus_SH + FA_minus_FH  || word), data=data4mixedmodels_words_scr[data4mixedmodels_words_scr$word_accuracy==1,], REML = TRUE, control=lmerControl(optimizer="bobyqa"))
summary(LMM_rt_scr_final)                        # model does converge, no singular fit -> model is identified

LMM_rt_scr_final_summary <- summary(LMM_rt_scr_final)

kable(LMM_rt_scr_final_summary$coefficients, caption = "Overview Fixed Effects LMM SCR") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "left")
```

<br>

Akin to the ANOVA omnibus test, it can be tested whehter the interaction response type x valence x SCR improves the model fit significantly. It does not. Thus, there is no omnibus interaction effect. The SCR does not modulate the priming effect and there is no relation between priming and SCR. This is also evident when the interaction is resolved in the model nested within each response condition (see below).
...
LMM_rt_scr_nested_summary <- summary(LMM_rt_scr_nested)

kable(LMM_rt_scr_nested_summary$coefficients, caption = "Overview Fixed Effects Nested LMM Word RT") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "left")
```
...
The results of the GLMM were quite the same. The only difference is that in the GLMM there is also a significant difference between response types FH_vs_SH and a significant effect of SCR (in the LMM the p value is at .067) in the full model. The nested GLMM does not converge anymore. In the past, it did converge. Recently, I decided to exclude word outlier trials also from the z standardization of the SCR. I rechecked, the convergence problems are not due to this change. I have no idea, what changed this. At least the results of no the non converging nested GLMM are consistent with the nested LMM (only one difference: in the nested GLMM there is now a trend for FH x SCR x valence with p = 0.073).  
...
By calculating the correlation [see below], I wanted to check whether the unexpected relation found in the original/problematic LMM (more SCR is related to less RT facilitation after a false alarm) can be seen on the level of aggregated data as well. 
