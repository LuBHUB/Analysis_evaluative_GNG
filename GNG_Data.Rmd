...
df_aggregated_over_subjects_gng_rt$response_type <- factor(df_aggregated_over_subjects_gng_rt$response_type, levels=c("FA","FH","SH")) 
...
df_aggregated_over_subjects_gng_prop$response_type <- factor(df_aggregated_over_subjects_gng_prop$response_type, levels=c("FA","FH","SH","CI")) 
...
tab_model(LMM_rt_gng_final,dv.labels = "GNG RT", pred.labels = pl,wrap.labels = 60, show.stat = TRUE, show.se = TRUE, string.se = "SE", show.loglik = TRUE, show.dev = TRUE, show.re.var = FALSE, show.ngroups = FALSE, show.icc = FALSE, show.r2 = FALSE)
```
...
kable(table_prop, caption = 'Proportion of Response Types') %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = T, position = "left")
```
...
#### Estimated Marignal Mean Plot

I obtained estimated marginal means (aka least square means or adjusted means) using the emmeans package. I can plot the back-transformed estimated marginal means for all conditions and the CIs. For inverse transformation, there is no convenient way for also back-transforming contrast estimates (and difference CIs) for the LMM table (for log transformation it would be possible). Hence, I will report the transformed values. In the plot of the estimated marginal means, the CIs of significantly differing conditions overlap considerably. 
