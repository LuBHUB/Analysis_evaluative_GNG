...
# reading in datafile
datafiles <- list.files("C:/Users/Luisa/PhD/1_PhD_Project/6_ModERN_Behavioral_Study/5_Analyses", pattern = "Data_Single_Trial") # replace pattern with .rda, if I also want to read in aggregated rda file
...
df_aggregated_per_subject_gng$response_type <- factor(df_aggregated_per_subject_gng$response_type, levels=c("SH","FH","FA","CI")) 
    


# aggregating over subjects per condition and calculate SE/CI corrected for within design (only needed for barplots)
source("C:/Users/Luisa/PhD/1_PhD_Project/6_ModERN_Behavioral_Study/5_Analyses/functions/summarySEwithinO.R")
df_aggregated_over_subjects_gng_rt <- summarySEwithinO(data4mixedmodels_gng, measurevar="gng_rt", withinvars="response_type", idvar="subjectID", conf.interval=.95)
df_aggregated_over_subjects_gng_rt$response_type <- factor(df_aggregated_over_subjects_gng_rt$response_type, levels=c("SH","FH","FA")) 

# for proportion of response_types, I can only apply function to the data already aggregated per subject (attaching proportion for response type to each trial in each subject makes no sense, as then I duplicate the data and artificially set the variance within one subject to 0)
df_aggregated_over_subjects_gng_prop <- summarySEwithinO(df_aggregated_per_subject_gng, measurevar="proportion", withinvars="response_type", idvar="subjectID", conf.interval=.95)
df_aggregated_over_subjects_gng_prop$response_type <- factor(df_aggregated_over_subjects_gng_prop$response_type, levels=c("SH","FH","FA","CI")) 
...
# reorder columns
df_wide_gng <- df_wide_gng[,c("subjectID","rt.SH","rt.FH","rt.FA","rt_inverse.SH","rt_inverse.FH","rt_inverse.FA","proportion.SH","proportion.FH","proportion.FA","proportion.CI")]
...
data4mixedmodels$gng_invalid_rt_fast <- FALSE
data4mixedmodels[data4mixedmodels$gng_invalid_rt == TRUE & data4mixedmodels$gng_rt < 100,]$gng_invalid_rt_fast <- TRUE

data4mixedmodels$gng_invalid_rt_slow <- FALSE
data4mixedmodels[data4mixedmodels$gng_invalid_rt == TRUE & data4mixedmodels$gng_rt > 700,]$gng_invalid_rt_slow <- TRUE

data4mixedmodels$gng_misses <- FALSE
data4mixedmodels[data4mixedmodels$gng_resp == 47,]$gng_misses <- TRUE

data4mixedmodels$gng_wrong_key <- FALSE
data4mixedmodels[data4mixedmodels$gng_resp == 48 | data4mixedmodels$gng_resp == 49,]$gng_wrong_key <- TRUE

# use same trials as in data4mixedmodels_words but not exclude word outliers here
df_outlier_gng <- ddply(data4mixedmodels, 
                          .(subjectID), 
                          summarise,
                          percent_gng_outliers_fast    = sum(gng_invalid_rt_fast)/length(subjectID)*100,
                          percent_gng_outliers_slow    = sum(gng_invalid_rt_slow)/length(subjectID)*100,
                          percent_gng_outliers_overall = sum(gng_invalid_rt)/length(subjectID)*100,
                          percent_gng_misses           = sum(gng_misses)/length(subjectID)*100,
                          percent_gng_wrong_key        = sum(gng_wrong_key)/length(subjectID)*100)

outlier_gng <- round(stat.desc(df_outlier_gng[,c(2:6)],basic=F), digits = 2)

kable(outlier_gng, caption = "GNG excluded trials (%)") %>%
...
#### Visualize RT per Subject

<br>

```{r plot gng RT per subject, fig.width=10, fig.height=8}

data4mixedmodels_gng$response_type <- factor(data4mixedmodels_gng$response_type, levels = c("SH","FH","FA"))

per_subj_gng <- ggplot(data4mixedmodels_gng, aes(x=response_type, y=gng_rt)) + geom_point(position="jitter") + ggtitle("GNG RT per Subject") + theme(plot.title = element_text(hjust = 0.5)) + facet_wrap( ~ subjectID, nrow=5)
per_subj_gng
```

<br>

#### Distribution & Prop. Resp. Types
...
* Fixed effects: Response type (SH, FH, FA)
    
* Random effects: Subjects

Trials with invalid RT to GNG stimulus or word were excluded from all analyses. SHs, FHs and FAs in the GNG task were included here. Trials with misses or responses with wrong keys in the GNG task were not included. 

GNG RT is not normally distributed. Distribution fits inverse gaussian distribution (see below). Hence, in line with the recommendation by Lo & Andrews (2015), I also calculated a GLMM with inverse gaussian distribution and identity link funktion. The results are in line with the LMM and the ANOVA results: there is a main effect of response type. Response times for FH and FA do not differ, responses for SH are slower than responses for FA. Initially, responses of FH and SH were not compared because these differ by definition and the number of contrasts is limited (levels of factor - 1). Below, I did compare all levels by changing the order of the factor levels.

<br>

#### Check Distribution 

GNG RT is not normally distributed (see above). The transfomed GNG RT Inverse approaches normal distribution (but the kurtosis is 3.4; the kurtosis of the residuals is even worse - 6.3). But as this deviation is mild and the analysis is of no interest and the GLMM and the ANOVA show the same results, results by means of an LMM may be reliable here. 
...
I chose sliding difference contrasts, which successively test neighboring factor levels against each other. The resulting estimates can be interpreted as the difference between subsequent factor levels. The advantage of this contrast is that the fixed effect intercept (group-level mean) is estimated as the grand average across all conditions, rather than the mean of a baseline condition, as for example for the default treatment contrast, which can cause troubles when using multiple predictors or interactions. Sliding constrasts are centered, meaning that the intercept reflects the grand mean. This is really important if the factors have more than 2 levels and when interactions between levels are tested. Compared to treatment contrasts, sliding contrasts allow better interpretation of interactions. For these contrasts, the predictor variables actually measure their main effects, i.e., their effects AVERAGED across the conditions of the other factor. Initially, I decided to compare FA with FH and SH (we can only compare n-1 factor levels in each model). Comparison FH and SH makes not much sense, because these differ per definition. Later, I decided to actually compare all levels, by changing the order of the factor levels.
...
names(data4mixedmodels_gng)[(ncol(data4mixedmodels_gng)-2):ncol(data4mixedmodels_gng)] <- c("Grand Mean", "FH_minus_SH","FA_minus_FH")
```
...
```{r run LMM RT, echo = 1} 
LMM_rt_gng_final <- lmer(gng_rt_inverse ~ FH_minus_SH + FA_minus_FH + (1 + FH_minus_SH + FA_minus_FH  | subjectID), data=data4mixedmodels_gng, REML = TRUE, control=lmerControl(optimizer="bobyqa"))

tab_model(LMM_rt_gng_final,dv.labels = "GNG RT", wrap.labels = 60, show.stat = TRUE, show.se = TRUE, string.se = "SE", show.loglik = TRUE, show.dev = TRUE, show.re.var = FALSE, show.ngroups = FALSE, show.icc = FALSE, show.r2 = TRUE)
```

<br>

To test all three levels of	the factor response type against each other, we ran models with two different factor-level orders (i.e., FH, FA, SH and FA, FH, SH). The result is: FH < SH.

```{r LMM RT 2, echo = 13, fig.width=10, fig.height=6}

# make categorical variables factors
data4mixedmodels_gng$response_type <- factor(data4mixedmodels_gng$response_type, levels=c("SH","FA","FH")) 
...
mm_c    <- model.matrix( ~ response_type, data4mixedmodels_gng) # stick in model matrix 3 columns
data4mixedmodels_gng[,((ncol(data4mixedmodels_gng)+1):(ncol(data4mixedmodels_gng)+3))] <- mm_c
names(data4mixedmodels_gng)[(ncol(data4mixedmodels_gng)-2):ncol(data4mixedmodels_gng)] <- c("Grand Mean", "FA_minus_SH","FH_minus_FA")

LMM_rt_gng_final2 <- lmer(gng_rt_inverse ~ FA_minus_SH + FH_minus_FA + (1 + FA_minus_SH + FH_minus_FA  | subjectID), data=data4mixedmodels_gng, REML = TRUE, control=lmerControl(optimizer="bobyqa"))

tab_model(LMM_rt_gng_final2,dv.labels = "GNG RT", wrap.labels = 60, show.stat = TRUE, show.se = TRUE, string.se = "SE", show.loglik = TRUE, show.dev = TRUE, show.re.var = FALSE, show.ngroups = FALSE, show.icc = FALSE, show.r2 = TRUE)


# undo change in order of factor levels
data4mixedmodels_gng$response_type <- factor(data4mixedmodels_gng$response_type, levels=c("SH", "FH", "FA")) 
```


The results are: SH > FA/FH, FH = FA. This is in line with the results obtained by the ANOVA post hoc tests. 
...
* Q-Q plot looks not really great; quite a bit off at the extremes (kurtosis is 6.3, skewness is -0.5) -> normality not really given
...
skewness <- round(skewness(resid(LMM_rt_gng_final)),digits = 2) 
kurtosis <- round(kurtosis(resid(LMM_rt_gng_final)),digits = 2) 
...
I calculated univariate one-way repeated measures ANOVAs, for the transformed RT inverse (to violate the assumption of normality of residuals a bit less) containing the within-subjects factor response type (levels SH, FH, FA) and for proportion of response type with the within-subjects factor response type (levels SH, FH, FA, CI). For RT, I also computed an ANOVA with untransformed RTs. The results are qualitatively the same and not reported here.
...
kable(round(anova_proportion_posthoc_response_type$p.value,digits=3), caption = "Proportion: p-values")  %>%
 kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "float_right")  %>%
 footnote(general = "P value adjusted with Holm method.")


# Get t and p values for proportion
source("C:/Users/Luisa/PhD/1_PhD_Project/6_ModERN_Behavioral_Study/5_Analyses/functions/pairwise.t.test.with.t.and.df.R")
result <- pairwise.t.test.with.t.and.df(df_aggregated_per_subject_gng$proportion,df_aggregated_per_subject_gng$response_type, p.adjust.method="holm",paired=TRUE)

t_values <- round(result[[5]],digits = 2)
dfs      <- round(result[[6]],digits = 2)

kable(cbind(t_values,dfs), caption = "Proportion: t-values and dfs")   %>%
 add_header_above(c(" ", "t-values" = 3, "dfs" = 3)) %>%
 kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "right")
```
...
```{r GNG ANOVAs effect sizes, message=FALSE, fig.width=10}

### calculate effect size for proportion post hoc tests
library("rstatix")

eff_sizes_RT <- df_aggregated_per_subject_gng  %>% cohens_d(rt_inverse ~ response_type, paired = TRUE)
eff_sizes_RT$effsize <- round(eff_sizes_RT$effsize,digits = 2)

kable(eff_sizes_RT[,c(2:7)], caption = "Effect Sizes for GNG RT")  %>%
 kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "float_left")

eff_sizes_proportion <- df_aggregated_per_subject_gng  %>% cohens_d(proportion ~ response_type, paired = TRUE)
eff_sizes_proportion$effsize <- round(eff_sizes_proportion$effsize,digits = 2)

kable(eff_sizes_proportion[,c(2:7)], caption = "Effect Sizes for Proportion of Response Types")  %>%
 kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "float_right")
```
...
barplot_rt <- ggplot(df_aggregated_over_subjects_gng_rt,aes (x = response_type,y = gng_rt)) +
  geom_bar(stat="identity", position=position_dodge(), fill = "wheat2", width = 0.7) +                                                       
  geom_errorbar(aes(ymax = gng_rt + ci, ymin = gng_rt - ci), position = position_dodge(width=0.95), width=0.1) +    
  ...
  geom_signif(y_position=c(360,360,380), xmin=c(1.02,2.02,1.02), xmax=c(1.98, 2.98,2.98),annotation=c("***","NS","***"), tip_length=0.02) 
 

barplot_cond <- ggplot(df_aggregated_over_subjects_gng_prop,aes (x = response_type,y = proportion)) +
  geom_bar(stat="identity", position=position_dodge(), fill = "wheat2") +                                                       
  geom_errorbar(aes(ymax = proportion + ci, ymin = proportion - ci), position = position_dodge(width=0.95), width=0.1) +  
  ...
  theme(legend.text=element_text(size=15)) +
  theme(plot.title = element_text(size = 20, hjust = 0.5)) +
  geom_signif(y_position=c(45,45,45), xmin=c(1.02, 2.02,3.02), xmax=c(1.98, 2.98,3.98),annotation=c("***","***","***"), tip_length=0.02) +
  geom_signif(y_position=c(49,53,57), xmin=c(1.02, 2.02,1.02), xmax=c(2.98, 3.98,3.98),annotation=c("***","NS","***"), tip_length=0.02) 


grid.arrange(barplot_rt,barplot_cond,nrow=1,top = textGrob('Behavioral Performance Go/No-Go Task', gp = gpar(fontsize = 20)))
...
Error bars depict 95% CI. Values are adjusted for within-participant designs following Morey (2008). Calculation of error bars for the RT was based on the non-aggregated data, as this better reflects the LMM approach. Computing error bars based on the aggregated data per subject yields different results, as in the mean of the group means it would not be considered that subjects have different number of events per condition; this more resembles the ANOVA approach, because it assumes that each subject mean is equally reliable / relies on the same amount of information, which is not true. For the Proportion of Response Type, I can only calculate error bars based on the data already aggregated per subject. Attaching proportion for response type to each trial in each subject makes no sense, as then I duplicate the data and artificially set the variance within one subject to 0.
...
df_aggregated_over_subjects_gng_rt$response_type <- factor(df_aggregated_over_subjects_gng_rt$response_type, levels=c("SH","FH","FA","CI")) 
table_rt <- df_aggregated_over_subjects_gng_rt[,c("response_type","N","gng_rt","sd","se","ci")]
table_rt[,c("gng_rt","sd","se","ci")] <- round(table_rt[,c("gng_rt","sd","se","ci")], digits = 2)
table_rt <- t(table_rt)
rownames(table_rt) <- c("","N","M","SD","SE","95% CI")

kable(table_rt[,c(3,2,1)], caption = 'Reaction Time') %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = T, position = "left")

table_prop <- df_aggregated_over_subjects_gng_prop[,c("response_type","N","proportion","sd","se","ci")]
table_prop[,c("proportion","sd","se","ci")] <- round(table_prop[,c("proportion","sd","se","ci")], digits = 2)
table_prop <- t(table_prop)
rownames(table_prop) <- c("","N","M","SD","SE","95% CI")
table_prop <- table_prop[,c(2,3,4,1)]

kable(table_prop[,c(3,2,1,4)], caption = 'Proportion of Response Types') %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = T, position = "left")
```

Note: The RT values here are based on the non-aggregated single trial data. This corresponds to the LMM approach and my plots. The Proportion of response types is based on the data first aggregated per subject and then over subjects.

<br>

#### Estimated Marginal Means Plot/Table
...
# emm_options(pbkrtest.limit = 11028)
# emm_options(lmerTest.limit = 11028)
# 
# # If I want to back transforn the inverse transformation, I can only do 1/RT
# tran <- make.tran("boxcox",-1)
# LMM4plot_gng <- with(tran,
#                      lmer(linkfun(gng_rt) ~ response_type + (1 + SH_minus_FA + FA_minus_FH  | subjectID), data=data4mixedmodels_gng, REML = TRUE, control =
#                           lmerControl(optimizer="bobyqa")))
# 
# start_time = Sys.time()
# Estimated_Means_GNG_RT = as.data.frame(emmeans(LMM4plot_gng, ~ response_type, type = "response"))
# end_time = Sys.time()
# end_time - start_time # Time difference of 9 mins
# Estimated_Means_GNG_RT$response_type <- factor(Estimated_Means_GNG_RT$response_type, levels = c("FA","FH","SH"))
# setwd("C:/Users/Luisa/PhD/1_PhD_Project/6_ModERN_Behavioral_Study/5_Analyses")
# save(Estimated_Means_GNG_RT,file="Estimated_Means_GNG_RT.rda")

load(file = "Estimated_Means_GNG_RT.rda")

# reorder factor levels 
Estimated_Means_GNG_RT$response_type <- factor(Estimated_Means_GNG_RT$response_type, levels = c("SH", "FH", "FA"))

plot_est_means_rt <- ggplot(Estimated_Means_GNG_RT,aes (x = response_type,y = response)) +
  geom_bar(stat="identity", position=position_dodge(0.9), size=1, width=0.8, color='black') +                                       
  geom_errorbar(aes(ymax = upper.CL, ymin = lower.CL), position = position_dodge(width=0.9), width=0.2) +    
  xlab("Response Type") + ylab("Estimates Marginal Means RT (ms)") +                                                                
  theme_bw(base_size = 20) +theme(legend.position = 'top',panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme(axis.title = element_text(size = 15, hjust = 0.5)) +                                                   
  scale_fill_manual(values=c("grey", "white")) 
plot_est_means_rt


kable(cbind(Estimated_Means_GNG_RT[c(3,1,2),1],round(Estimated_Means_GNG_RT[c(3,1,2),c(2:6)], digits = 2)), col.names = c("response type", "Mean", "SE", "df", "lower CI", "upper CI"), caption = 'Estimated Marginal Means GNG RT') %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = T, position = "left")
```
