...
tab_model(LMM_rt_final, order.terms = c(1,2,6,3,7,4,8,5), dv.labels = "RT [-1000/RT(ms)]] Word Categorization", pred.labels = pl,wrap.labels = 60, show.stat = TRUE, show.se = TRUE, string.se = "SE", show.loglik = TRUE, show.dev = TRUE, show.re.var = FALSE, show.ngroups = FALSE, show.icc = FALSE, show.r2 = FALSE)
```
<br>
...
tab_model(LMM_rt_nested,dv.labels = "RT [-1000/RT(ms)]] Word Categorization", pred.labels = pl,wrap.labels = 60, show.stat = TRUE, show.se = TRUE, string.se = "SE", show.loglik = TRUE, show.dev = TRUE, show.re.var = FALSE, show.ngroups = FALSE, show.icc = FALSE, show.r2 = FALSE)
```
...
The results are also in line with the GLMM on the raw word RT. The only difference is that in the GLMM there is also a significant difference between response types FH_vs_SH. (But only if I run the full model that corresponds to the nested model that would converge. It is not signitificant in the really full GLMM). This is of no interest. It is probably caused by the sparse random structure in the GLMM which resulted from convergence problems. But due to the presence of a disordinal interaction effect, the main effects cannot be sensibly interpreted anyway.
...
tab_model(GLMM_acc_final,dv.labels = "Correctness of Word Categorization", pred.labels = pl,wrap.labels = 60, show.stat = TRUE, show.se = TRUE, string.se = "SE", show.loglik = TRUE, show.dev = TRUE, show.re.var = FALSE, show.ngroups = FALSE, show.icc = FALSE, show.r2 = FALSE)
```
<br>
...
tab_model(GLMM_acc_nested,dv.labels = "Correctness of Word Categorization", pred.labels = pl,wrap.labels = 60, show.stat = TRUE, show.se = TRUE, string.se = "SE", show.loglik = TRUE, show.dev = TRUE, show.re.var = FALSE, show.ngroups = FALSE, show.icc = FALSE, show.r2 = FALSE)
```
...
Aarts et al. (2012) only used 3 levels of the factor response type (FA, FH, SH). Aarts et al. (2012) only used 2 levels of the factor response type (FA, FH). They agued that the word categorization was not influenced by preceding CIs and SHs and thus did not include these factor levels in the ANOVA. To my mind, it makes sense to include all levels. But my power analysis was based on their analysis and for matters of replication it may be reasonable to calculate their model. Here I use their approach (study 2012) to have a direct comparison of results. For the ANOVA on RT and on Accuracy, there is a significant main effect of response type and a significant interaction between response type and word valence. For the ANOVA on Accuracy, there is a significant main effect of valence. Post hoc pairwise *t* tests show that RT for negative and positive words differs significantly after FA, FH and SH and that Accuracy for negative and positive differs significantly after FA, but not after FH or SH. The results are in line with the findings reported by Aarts et al. (2012, 2013). In contrast to the original study we also found that  RT for negative and positive words differs also after FH (was only a trend in original paper) and SH. Furthermore, the main effects for Accuracy were not significant in the original study (response type was a trend, valence n.s.), but these are of no interest anyway, as an interaction is present. 

```{r word ANOVAs Aarts, fig.width=10, fig.height=6}
options(contrasts=c("contr.sum","contr.poly")) # to adhere to the sum-to-zero convention for effect weights, always do this 

df_aggregated_per_subject_aarts <- df_aggregated_per_subject[df_aggregated_per_subject$response_type == "FA" | df_aggregated_per_subject$response_type == "FH" | df_aggregated_per_subject$response_type == "SH",]

anova_rt_aarts <- aov_ez(id = "subjectID", dv = "rt_inverse", data = df_aggregated_per_subject_aarts, within = c("response_type","word_valence"))
apa_anova_rt_aarts <- apa_print(anova_rt_aarts)

anova_accuracy_aarts <- aov_ez(id = "subjectID", dv = "percent_correct_responses", data = df_aggregated_per_subject_aarts, within = c("response_type","word_valence"))
apa_anova_accuracy_aarts <- apa_print(anova_accuracy_aarts)


kable(apa_anova_rt_aarts$table, caption = "Reaction Time Word Categorization")  %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "float_left")

kable(apa_anova_accuracy_aarts$table, caption = "Accuracy Word Categorization")  %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "float_right")


# Post Hoc Tests
anova_rt_posthoc_response_type_aarts         <- pairwise.t.test(df_aggregated_per_subject_aarts$rt, df_aggregated_per_subject_aarts$response_type, p.adjust.method="holm", paired=TRUE)

anova_accuracy_posthoc_response_type_aarts   <- pairwise.t.test(df_aggregated_per_subject_aarts$percent_correct_responses, df_aggregated_per_subject_aarts$response_type, p.adjust.method="holm",paired=TRUE)

anova_rt_posthoc_interaction_aarts           <- pairwise.t.test(df_aggregated_per_subject_aarts$rt, df_aggregated_per_subject_aarts$condition, p.adjust.method="holm", paired=TRUE)

anova_accuracy_posthoc_interaction_aarts     <- pairwise.t.test(df_aggregated_per_subject_aarts$percent_correct_responses, df_aggregated_per_subject_aarts$condition, p.adjust.method="holm",paired=TRUE)


 kable(anova_rt_posthoc_response_type_aarts$p.value, caption = "RT: Post Hoc Test Response Type")  %>%
   kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "float_left")  %>% footnote(general = "P value adjusted with Holm method.")

 kable(anova_accuracy_posthoc_response_type_aarts$p.value, caption = "Accuracy: Post Hoc Test Response Type")  %>%
   kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "float_right")  %>% footnote(general = "P value adjusted with Holm method.")

 kable(anova_rt_posthoc_interaction_aarts$p.value, caption = "RT: Post Hoc Test Response Type x Word Valence")  %>%
   kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "float_left")  %>% footnote(general = "P value adjusted with Holm method.")

 kable(anova_accuracy_posthoc_interaction_aarts$p.value, caption = "Accuracy: Post Hoc Test Response Type x Word Valence")  %>%
   kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 10, full_width = F, position = "float_right")  %>% footnote(general = "P value adjusted with Holm method.")
```
...
#### Estimated Marignal Means Plots

I obtained estimated marginal means (aka least square means or adjusted means) using the emmeans package. I can plot the back-transformed estimated marginal means for all conditions and the CIs. For inverse transformation, there is no convenient way for also back-transforming contrast estimates (and difference CIs) for the LMM table (log and logit transforms are exceptional cases in that there is a valid way to back-transform contrasts: https://github.com/rvlenth/emmeans/blob/master/R/contrast.R). Hence, I will report the transformed values. In the plot of the estimated marginal means, the CIs of significantly differing conditions overlap considerably. 
...
```{r estimated mean plots, fig.width=10}
...
plot_est_means_rt <- ggplot(emm2,aes (x = response_type,y = response, fill = valence)) +
  ...
  xlab("Preceding Response Type") + ylab("Estimates Marginal Means RT (ms)") +                                                                  
  guides(fill=guide_legend(title="Word Valence")) +
...
Estimated_Means_Priming_Acc$response_type <- factor(Estimated_Means_Priming_Acc$response_type, levels = c("FA","FH","SH","CI"))

plot_est_means_acc <- ggplot(Estimated_Means_Priming_Acc,aes (x = response_type,y = prob, fill = valence)) +
  geom_bar(stat="identity", position=position_dodge(0.9), size=1, width=0.8, color='black') +                                                       
  geom_errorbar(aes(ymax = asymp.UCL, ymin = asymp.LCL), position = position_dodge(width=0.9), width=0.2) +    
  xlab("Preceding Response Type") + ylab("Estimates Marginal Means Hit Probability") +                                                                  
  guides(fill=guide_legend(title="Word Valence")) +
  theme_bw(base_size = 20) +theme(legend.position = 'top',panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme(axis.title = element_text(size = 15, hjust = 0.5)) +                                                   
  scale_fill_manual(values=c("grey", "white")) 
  ...
  grid.arrange(legend, nrow=2,arrangeGrob(plot_est_means_rt,plot_est_means_acc,nrow=1),heights=c(1, 10))
```
<br>
<br>
<br>
